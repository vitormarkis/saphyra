...
Existe uma definição de boa abstração que sempre levo comigo, "abstrações que sequestram o problema, resolvem, e te dão uma API para interagir com ele são abstrações ruins, ao passo que abstrações que te dão primitivos para você resolver o problema são abstrações boas". Boas soluções são criadas em cima de boas abstrações.
...
- **Nunca mais escrever loading states**: XXX possui transitions, o que significa que toda ação assíncrona que você queira executar, gere um loading state o qual você pode se inscrever usando hooks.
